  ...
  if (!exercise) {
    return (
      <section
        ...
        aria-labelledby="function-lab-complete-title"
      >
        ...
        <div>
          ...
          <p>
            You passed values in, returned results, kept temporary state local,
            and built a closure that remembers. Those four ideas support every
            larger JavaScript program.
          </p>
    ...
    >
      <header className="function-lab-progress">
        ...
        <div
          ...
          role="progressbar"
          aria-label="Function exercises completed"
          aria-valuemin={0}
          aria-valuemax={JAVASCRIPT_FUNCTION_EXERCISES.length}
      ...
      <div className="function-lab-grid">
        <aside className="function-lab-lesson">
          <p className="eyebrow">Build one reusable behavior</p>
          <h2 id="function-lab-title">{exercise.title}</h2>
          ...
          <ol className="function-lab-path" aria-label="Function concepts">
            {JAVASCRIPT_FUNCTION_EXERCISES.map((item, index) => (
              <li
                className={
                  index === exerciseIndex
                    ? "is-current"
        ...
        <div className="function-lab-editor">
          ...
          <GuidedJavaScriptCodeEditor
            ...
            label="JavaScript functions and scope code"
            value={code}
            onChange={(value) => {
              setCode(value);
              setCheckState({
          ...
          <div className="function-lab-actions">
            ...
            {isPassed ? (
              <button
                ...
                type="button"
              >
                {exercise.number === JAVASCRIPT_FUNCTION_EXERCISES.length
                  ? "Finish the lab"
                  : `Continue to ${JAVASCRIPT_FUNCTION_EXERCISES[exerciseIndex + 1].concept}`}
                <span aria-hidden="true">→</span>
              </button>
            ) : (
              <button
