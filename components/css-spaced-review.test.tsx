...
import { CSS_SPACED_REVIEW_ITEMS } from "@/lib/css-spaced-review";
import { CssSpacedReview } from "./css-spaced-review";

describe("CssSpacedReview", () => {
  afterEach(() => {
    cleanup();
    vi.restoreAllMocks();
  });
  ...
  it("teaches after each choice and saves only the bounded result", async () => {
    ...
    render(<CssSpacedReview initialResult={null} />);

    for (const [index, item] of CSS_SPACED_REVIEW_ITEMS.entries()) {
      const choice =
        index === 0
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("heading", {
          name: "Your next CSS review is set for Aug 22.",
        }),
      ).toBeInTheDocument(),
    );
    expect(fetchSpy).toHaveBeenCalledWith("/api/practice/css/spaced-review", {
      method: "POST",
    ...
    expect(
      screen.getByRole("link", { name: "Continue to the capstone" }),
  ...
  it("keeps completion hidden and retryable after a failed save", async () => {
    ...
    expect(screen.queryByText("Private CSS review saved")).not.toBeInTheDocument();
