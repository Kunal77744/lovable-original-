...
import JavaScriptLinkedListsPage, { metadata } from "./page";
...
describe("JavaScriptLinkedListsPage", () => {
  ...
  it("redirects signed-out visitors before reading private progress", async () => {
    ...
    await expect(JavaScriptLinkedListsPage()).rejects.toThrow("NEXT_REDIRECT");
    expect(redirectMock).toHaveBeenCalledWith(
      "/account?mode=signin&next=/practice/linked-lists",
    );
  ...
  it("renders the private four-exercise lab at the first unfinished step", async () => {
    getSession.mockResolvedValue({
      user: { id: "linked-list-learner" },
    } as Awaited<ReturnType<typeof auth.api.getSession>>);
    getCompletedExerciseIds.mockResolvedValue(["connect-the-next-node"]);

    render(await JavaScriptLinkedListsPage());

    expect(
      screen.getByRole("heading", {
        name: "Follow the links without losing the list.",
      }),
    ).toBeInTheDocument();
