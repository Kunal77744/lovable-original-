...
describe("playground route", () => {
  ...
  it("scopes every restored file to the current account", async () => {
    ...
    await expect(firstResponse.json()).resolves.toMatchObject({
      file: { code: "console.log('student a')" },
    });
    await expect(secondResponse.json()).resolves.toMatchObject({
      file: { code: "console.log('student b')" },
    });
  ...
  it("saves exact code for the signed-in account only", async () => {
    ...
    expect(mocks.savePlaygroundFile).toHaveBeenCalledWith("student-a", code);
